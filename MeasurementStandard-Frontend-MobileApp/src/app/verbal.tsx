...
export default function VerbalScreen() {
  ...
  return (
    <View className="flex-1 bg-[#F8FAFC]">
      ...
      <ScreenHeader title="القدرات اللفظية" subtitle="تدريبات المفردات والاستيعاب اللغوي" />

      {isLoading ? (
        <LoadingView />
      ) : error ? (
        <ErrorState message={error} onRetry={load} />
      ) : verbalSections.length === 0 ? (
        <EmptyState
          icon="book-open"
          title="لا توجد تدريبات لفظية حالياً"
          subtitle="سيتم إضافة تدريبات القدرات اللفظية قريباً"
        />
      ) : (
        <FlatList
          data={verbalSections}
