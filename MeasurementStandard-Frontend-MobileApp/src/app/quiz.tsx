...
export default function QuizScreen() {
  ...
  const handleSelect = (choiceId: string) => {
    if (!safeQuestions[currentIndex]) return;
    setSelected((prev) => ({
      ...prev,
      [safeQuestions[currentIndex].id]: choiceId,
    }));
  };
  ...
  const handleSubmit = async () => {
    ...
    if (answeredList.length !== safeQuestions.length) {
      setSubmitError(
        `لم تتم الإجابة على ${
          safeQuestions.length - answeredList.length
        } سؤال - أجب عنها ثم أرسل`
      );
      return;
    }
    setSubmitting(true);
    ...
    return (
      <View className="flex-1 bg-[#F8FAFC]">
        ...
        <View className="flex-1 items-center justify-center px-8">
          ...
          <Text className="text-[#2B2D5C] font-extrabold text-2xl mb-2">
            اختبار مكتمل بنجاح
          </Text>
          <Text className="text-[#64748B] text-sm font-semibold text-center mb-8">
            أجبت بشكل صحيح على {result.correct_count} من {result.total_questions}{" "}
            أسئلة
          </Text>

          <View className="flex-row-reverse items-center justify-center mb-10">
            <View className="items-center bg-white rounded-2xl px-8 py-4 border border-gray-100 ml-3">
              <Text className="text-[#2B2D5C] font-black text-xl">
                {result.correct_count}
              </Text>
              <Text className="text-[#94A3B8] text-xs font-semibold mt-1">
                إجابات صحيحة
              </Text>
            </View>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.85}
          >
            <Text className="text-[#2B2D5C] font-bold text-base">
              سجل الاختبارات
            </Text>
          </TouchableOpacity>
        </View>
    ...
    <View className="flex-1 bg-[#F8FAFC]">
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          subtitle="لم يتم إضافة أسئلة لهذا القسم بعد"
        />
      ) : (
        <ScrollView
          className="flex-1"
        ...
        >
          <View className="flex-row-reverse items-center justify-between mb-4">
            <View className="flex-row-reverse items-center">
              ...
              <Text className="text-[#059669] text-xs font-bold mr-1 ml-1">
                {answeredCount}/{safeQuestions.length} أجبت
              </Text>
            </View>
            <Text className="text-[#94A3B8] text-xs font-semibold">
              سؤال {currentIndex + 1} من {safeQuestions.length}
            </Text>
          </View>
          ...
          {question ? (
            <>
              <View className="bg-white rounded-2xl p-5 border border-gray-100 mb-6">
                <Text
                  className="text-[#2B2D5C] font-extrabold text-lg leading-7"
                ...
                {(question.choices || []).map((choice, idx) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      activeOpacity={0.85}
                      className={`flex-row-reverse items-center rounded-2xl p-4 mb-3 border ${
                        isSelected
                    ...
                    >
                      ...
                      <Text
                        className={`flex-1 font-bold text-base ${
                          isSelected ? "text-white" : "text-[#2B2D5C]"
                        }`}
                        style={{ textAlign: "right" }}
                      >
                        {choice.content}
          ...
          {submitError ? (
            <Text className="text-rose-600 text-sm font-bold text-center mt-2 mb-2">
              {submitError}
            </Text>
          ) : null}
          ...
          >
            {!isLast ? (
              <TouchableOpacity
                onPress={() =>
                  setCurrentIndex((i) => Math.min(safeQuestions.length - 1, i + 1))
                }
