...
export class AuthService {
  ...
  async register(
    registerDto: RegisterDto & { profile_picture?: string | null },
  ) {
  ...
  async verifyCurrentPassword(userId: string, passwordToCheck: string) {
    ...
    if (!user || !user.password_hash) {
      throw new UnauthorizedException(
        'المستخدم غير موجود أو مسجل عبر Google فقط',
      );
    }
  ...
  private async generateTokens(user: User) {
    ...
    const accessExpiresIn =
      this.configService.get<number>('ACCESS_EXPIRE_IN') ??
      Number(process.env.ACCESS_EXPIRE_IN);
    const refreshExpiresIn =
      this.configService.get<number>('REFRESH_EXPIRE_IN') ??
      Number(process.env.REFRESH_EXPIRE_IN);
  ...
  async refreshTokens(userId: string, refreshToken: string) {
    ...
    if (!user || !user.refresh_token)
      throw new UnauthorizedException('غير مصرح بالوصول');
    ...
    if (!refreshTokenMatches)
      throw new UnauthorizedException('توكن التجديد غير صالح');
